...
// --- 5. Admin Dashboard with Calendar ---
const AdminView = ({ onBack }) => {
  ...
  const handleLogout = async () => {
    try { await signOut(auth); await signInAnonymously(auth); onBack(); } catch (e) { onBack(); }
  };
  ...
  const renderCalendar = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(<div key={`empty-${i}`} className="h-14 bg-stone-50/50 border border-stone-100"></div>);
    }
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (isFullDayOff) { bgColor = 'bg-red-50'; statusIcon = '🔴'; }
      else if (isFullyBooked) { bgColor = 'bg-stone-100'; statusIcon = '🈵'; }
      else if (isPartialOff) { bgColor = 'bg-orange-50'; statusIcon = '🟡'; }
      else { bgColor = 'bg-white hover:bg-green-50'; }

      days.push(
        <div key={d} onClick={() => setSelectedDate(dateStr)} className={`h-14 border border-stone-100 p-1 cursor-pointer transition-all flex flex-col justify-between ${bgColor} ${isSelected ? 'ring-2 ring-rose-400 z-10' : ''}`}>
          <div className="flex justify-between items-start"><span className={`text-sm font-medium ml-1 ${isSelected ? 'text-rose-600' : 'text-stone-600'}`}>{d}</span><span className="text-[10px]">{statusIcon}</span></div>
          <div className="text-[10px] text-stone-400 text-center">{dayAppts.length > 0 && `${dayAppts.length} 預約`}</div>
        </div>
      );
    }
    return days;
  ...
  return (
    <div className="min-h-screen bg-stone-100 pb-20">
      <ConfirmModal isOpen={modalOpen} onClose={() => setModalOpen(false)} onConfirm={async () => { if (targetApptId && db) { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'jm_appointments', targetApptId), { status: 'cancelled' }); setModalOpen(false); }}} title="確定取消預約？" message="取消後，該時段將釋出給其他客人預約。" />
      <div className="bg-stone-800 text-stone-200 p-4 sticky top-0 z-50 flex justify-between items-center shadow-lg"><span className="font-bold tracking-widest flex items-center gap-2"><Lock size={16}/> 後台管理</span><button onClick={handleLogout} className="text-xs border border-stone-600 px-3 py-1 rounded hover:bg-stone-700 flex items-center gap-1"><LogOut size={12}/> 登出</button></div>
      <div className="max-w-5xl mx-auto p-4 grid lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 space-y-4">
          <div className="bg-white rounded-2xl shadow-sm p-4">
            <div className="flex justify-between items-center mb-4 px-2"><button onClick={() => changeMonth(-1)} className="p-2 hover:bg-stone-100 rounded-full"><ChevronLeft size={20}/></button><h2 className="text-lg font-bold text-stone-800 flex items-center gap-2"><CalendarIcon size={18} className="text-rose-400"/> {currentDate.getFullYear()} 年 {currentDate.getMonth() + 1} 月</h2><button onClick={() => changeMonth(1)} className="p-2 hover:bg-stone-100 rounded-full"><ChevronRight size={20}/></button></div>
            <div className="grid grid-cols-7 text-center mb-2">{['日', '一', '二', '三', '四', '五', '六'].map(d => <div key={d} className="text-xs font-bold text-stone-400 py-1">{d}</div>)}</div>
            <div className="grid grid-cols-7 bg-stone-100 gap-[1px] border border-stone-200">{renderCalendar()}</div>
            <div className="mt-3 flex gap-4 text-xs text-stone-500 justify-center"><span className="flex items-center gap-1"><div className="w-3 h-3 bg-red-100 border border-stone-200"></div> 公休</span><span className="flex items-center gap-1"><div className="w-3 h-3 bg-orange-100 border border-stone-200"></div> 部分</span><span className="flex items-center gap-1"><div className="w-3 h-3 bg-white border border-stone-200"></div> 營業</span></div>
          </div>
        </div>
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white p-5 rounded-2xl shadow-sm">
            <h3 className="font-bold text-stone-700 mb-3 text-lg border-b pb-2">{selectedDate} 設定</h3>
            <div className="space-y-3">
              <div className="flex justify-between items-center bg-stone-50 p-3 rounded-lg"><span className="text-sm font-medium text-stone-600">全日狀態</span><button onClick={() => toggleBlock(selectedDate)} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${blockedDates[selectedDate] === 'ALL' ? 'bg-red-500 text-white shadow-md' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>{blockedDates[selectedDate] === 'ALL' ? '⛔ 已設公休' : '✅ 營業中'}</button></div>
              {blockedDates[selectedDate] !== 'ALL' && (<div><p className="text-xs text-stone-400 mb-2">關閉特定時段：</p><div className="grid grid-cols-3 gap-2">{FIXED_SLOTS.map(slot => { const isBlocked = Array.isArray(blockedDates[selectedDate]) && blockedDates[selectedDate].includes(slot); return (<button key={slot} onClick={() => toggleBlock(selectedDate, slot)} className={`py-2 rounded-lg text-xs font-medium border transition-all ${isBlocked ? 'bg-stone-200 text-stone-400 line-through border-transparent' : 'bg-white border-stone-200 text-stone-600 hover:border-rose-300 hover:text-rose-500'}`}>{slot}</button>) })}</div></div>)}
            </div>
          </div>
          <div className="bg-white p-5 rounded-2xl shadow-sm min-h-[300px]">
            <h3 className="font-bold text-stone-700 mb-4 flex items-center gap-2">📋 當日預約 <span className="bg-rose-100 text-rose-600 text-xs px-2 py-0.5 rounded-full">{selectedDateAppts.length}</span></h3>
            <div className="space-y-3">{selectedDateAppts.length === 0 ? (<div className="text-center py-10 text-stone-300 text-sm">今日尚無預約</div>) : (selectedDateAppts.map(app => (<div key={app.id} className="border border-stone-100 rounded-xl p-3 hover:shadow-sm transition bg-stone-50/50"><div className="flex justify-between items-start mb-2"><span className="font-mono font-bold text-lg text-stone-700">{app.time}</span><span className={`text-[10px] px-2 py-1 rounded-full ${app.status === 'cancelled' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>{app.status === 'cancelled' ? '已取消' : '已預約'}</span></div><div className="text-sm font-bold text-stone-800">{app.client.name}</div><div className="text-xs text-stone-500 mb-2">{app.serviceName}</div>{app.status !== 'cancelled' && (<div className="border-t border-stone-200 pt-2 mt-2 flex justify-between items-center"><span className="text-xs text-stone-400">Line: {app.client.line}</span><button onClick={() => { setTargetApptId(app.id); setModalOpen(true); }} className="text-xs text-red-400 hover:text-red-600 hover:underline">取消</button></div>)}</div>)))}</div>
          </div>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      setView('success');
    } catch(e) { console.error(e); alert('預約失敗，請稍後再試: ' + e.message); }
    setLoading(false);
  };

  const handleAdminLogin = async () => {
    setLoginError(''); setIsLoggingIn(true);
    try { await signInWithEmailAndPassword(auth, adminEmail, adminPassword); setView('admin'); } catch (error) { setLoginError('登入失敗，請確認 Firebase 帳號密碼'); }
    setIsLoggingIn(false);
  };

  if (initError) return <div className="p-8 text-red-500">系統初始化失敗：{initError}</div>;
  ...
  if (view === 'adminLogin') return (
    <div className="min-h-screen flex items-center justify-center bg-stone-100">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm text-center">
        ...
        <input type="email" value={adminEmail} onChange={e=>setAdminEmail(e.target.value)} className="w-full p-3 bg-stone-50 rounded mb-2 text-center outline-none border border-stone-200 focus:border-rose-400" placeholder="Email" />
        <input type="password" value={adminPassword} onChange={e=>setAdminPassword(e.target.value)} className="w-full p-3 bg-stone-50 rounded mb-4 text-center tracking-widest outline-none border border-stone-200 focus:border-rose-400" placeholder="Password" />
        {loginError && <p className="text-red-500 text-xs mb-3">{loginError}</p>}
        <button onClick={handleAdminLogin} disabled={isLoggingIn} className="w-full py-3 bg-stone-800 text-white rounded font-bold hover:bg-stone-700 transition-all">{isLoggingIn ? 'Verifying...' : 'LOGIN'}</button>
      ...
      <div className="relative z-10 animate-fade-in-up w-full max-w-md flex flex-col items-center">
        ...
        <div className="mb-6 w-48 h-48 rounded-full bg-cream-100 shadow-sm overflow-hidden border-4 border-white relative flex items-center justify-center">
          ...
          <div className="absolute inset-0 bg-stone-100 hidden flex-col items-center justify-center text-stone-400">
            ...
            <span className="text-xs font-bold mt-2">JM NAIL</span>
          </div>
        </div>
